...
class GenMRDataset(torch.utils.data.Dataset):
    """Dataset containing prostate MR images.
    ...
    def __init__(self, paths, img_size):
        ...
        for path in paths:
            self.mask_list.append(
                sitk.GetArrayFromImage(sitk.ReadImage(path)).astype(np.int32)
            )

        # number of patients and slices in the dataset
        self.no_patients = len(self.mask_list)
        self.no_slices = self.mask_list[0].shape[0]
        ...
        # standardize intensities based on mean and std deviation
        #self.train_data_mean = np.mean(self.mr_image_list)
        #self.train_data_std = np.std(self.mr_image_list)
        #self.norm_transform = transforms.Normalize(
         #   self.train_data_mean, self.train_data_std
    ...
    def __getitem__(self, index):
        """Returns the preprocessed MR image and corresponding segmentation
        for a given index.
        ...
        return self.img_transform((self.mask_list[patient][the_slice, ...] > 0).astype(np.int64))
    

class GenMRDataset2(torch.utils.data.Dataset):
    """Dataset containing prostate MR images.
    ...
    def __init__(self, paths, img_size):
        ...
        for path in paths:
            self.mr_image_list.append(
                (sitk.GetArrayFromImage(sitk.ReadImage(path / "mr_bffe.mhd"))*1000).astype(
                    np.int32
                )[0,...]
            )
            self.mask_list.append(
