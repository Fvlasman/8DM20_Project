...
DATA_DIR = Path(r"C:\Users\sejin\Documents\capita\TrainingData\TrainingData")
AUG_DATA_DIR = Path( r"C:\Users\sejin\Documents\capita\generated_images\generated_images")
...
aug_patients = [
    path
    for path in AUG_DATA_DIR.glob("*")
    if not any(part.startswith(".") for part in path.parts)
]
random.shuffle(patients)

all_patients = patients + aug_patients
...
aug_dataset = utils.GenMRDataset2(aug_patients, IMAGE_SIZE)

augmented_dataset = ConcatDataset([dataset, aug_dataset])
