...
atlas_1 = sitk.ReadImage("TrainingData-GroupAssignment\TrainingData\p102\mr_bffe.mhd")    
atlas_2 = sitk.ReadImage("TrainingData-GroupAssignment\TrainingData\p108\mr_bffe.mhd")        
atlas_3 = sitk.ReadImage("TrainingData-GroupAssignment\TrainingData\p116\mr_bffe.mhd")
atlas_4 = sitk.ReadImage("TrainingData-GroupAssignment\TrainingData\p128\mr_bffe.mhd")
atlas_5 = sitk.ReadImage("TrainingData-GroupAssignment\TrainingData\p129\mr_bffe.mhd")
...
for i in range(0, image_shape[0]):
    for j in range(0, image_shape[1]):
        for k in range(0, image_shape[2]):
            value_list[0] = image_1[i,j,k]
            value_list[1] = image_2[i,j,k]
            value_list[2] = image_3[i,j,k]
...
plt.title('Segmentation of p129', fontsize=10)
plt.axis('off')

fig.add_subplot(2,3,6)
plt.imshow(npa_zslice_atlas)
plt.title('Combined Segmentation by Averaging', fontsize=10)
plt.axis('off')
